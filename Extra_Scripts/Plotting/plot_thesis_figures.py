...
It saves the plots directly to the specified output directory as PNG image files.
"""
...
def get_pwrflw_files(pwrflw_dir):
    """
    Scans the given directory for HDF5 output files.
    If EliasH is in the path, it looks for .h5 files.
    Otherwise, it looks for files ending with 'pwrflw.h5'.
    ...
    if "EliasH" in pwrflw_dir:
        # EliasH files don't have the pwrflw suffix
        h5_files = sorted([f for f in all_files if f.endswith('.h5')])
    else:
        # Filter to only keep files ending with 'pwrflw.h5'
        h5_files = sorted([f for f in all_files if f.endswith('pwrflw.h5')])
    
    # If no files are found, try local fallback directory
    if not h5_files:
        print("No files found in primary directory. Checking local fallback...")
        fallback_files = glob.glob(os.path.join('../../local_destination', '*.h5'))
        if "EliasH" in pwrflw_dir:
            h5_files = sorted([f for f in fallback_files if f.endswith('.h5')])
        else:
            h5_files = sorted([f for f in fallback_files if f.endswith('pwrflw.h5')])
...
def plot_figure_4_2_a(pwrflw_dir, out_dir, preurbs, file_suffix, color, case_label, title=None):
    ...
    if not h5_files:
        print(f"Error: No HDF5 power flow files found for {case_label}!")
        return
        ...
        try:
            with pd.HDFStore(filepath, 'r') as store:
                if input_key in store and output_key in store and '/raw_data/buildings' in store:
                    ...
                    peak_in = np.max(np.abs(p_in))
                    peak_out = np.max(np.abs(p_out))
                    
                    if peak_in != 0:
                        rel = (peak_out - peak_in) / peak_in * 100.0
                        
                        # Identify and exclude outliers with very high relative change
                        if rel > 400:
                            print(f"\n[OUTLIER DETECTED AND EXCLUDED]")
                            print(f"File: {os.path.basename(filepath)}")
                            print(f"Buildings: {num_buildings} | Relative Change: {rel:.2f}%\n")
                            continue
                            
                        # Print grids with -100% relative change (0 demand) but still plot them
                        if rel <= -99.99:
                            print(f"[ZERO DEMAND DETECTED] File: {os.path.basename(filepath)} | Buildings: {num_buildings} | Rel Change: {rel:.2f}%")
    ...
    if not building_counts:
        print(f"No peak load data could be loaded. Skipping Figure 4.2 a) for {case_label}.")
        return
...
def plot_figure_4_2_b(pwrflw_dir, out_dir, preurbs, file_suffix, colors, case_label, title=None):
    """
    Generates Figure 4.2 b): Histogram of Grid-Wide Maximum Upper and Lower Voltage Deviations.
    
    This function reads the voltage magnitudes from the power flow output files
    and plots the histogram of grid-wide maximum and minimum voltage magnitudes over a year.
    """
    h5_files = get_pwrflw_files(pwrflw_dir)
    if not h5_files:
        print(f"Error: No HDF5 power flow files found for {case_label}!")
        ...
        try:
            ...
            with pd.HDFStore(filepath, 'r') as store:
                ...
                if vm_key in store:
                    ...
                    max_voltages.append(np.nanmax(vm.values))
                    min_voltages.append(np.nanmin(vm.values))
                    processed_count += 1
        except Exception as e:
...
def plot_figure_4_2_b_filtered(pwrflw_dir, out_dir, preurbs, file_suffix, colors, case_label, title=None):
    """
    Generates Figure 4.2 b): Histogram of Grid-Wide Maximum Upper and Lower Voltage Deviations.
            ...
            with pd.HDFStore(filepath, 'r') as store:
                if vm_key in store and '/raw_data/buildings' in store:
                    ...
                    if not vm_filtered.empty:
                        max_voltages.append(np.nanmax(vm_filtered.values))
                        min_voltages.append(np.nanmin(vm_filtered.values))
                        processed_count += 1
                    else:
...
def plot_figure_4_1(pwrflw_dir, out_dir, preurbs, file_suffix, colors, case_label, title=None):
    """
    ...
    if not p_series_list:
        print(f"No active/reactive power data could be loaded. Skipping Figure 4.1 for {case_label}.")
        return
    ...
    row_labels = [
        ('Active Power P Import', 'P', 'Norm. Active Power Import $P_i(t) / \\langle |S_i| \\rangle$', 'Norm. Active Power Import $P_i / \\langle \\max |S| \\rangle$', c1),
        ('Reactive Power Q Import', 'Q', 'Norm. React. Power Import $Q_i(t) / \\langle |S_i| \\rangle$', 'Norm. React. Power Import $Q_i / \\langle \\max |S| \\rangle$', c2),
        ('Apparent Power |S| Load', 'S', 'Norm. Apparent Power Load $|S_i(t)| / \\langle |S_i| \\rangle$', 'Norm. Apparent Power Load $S_i / \\langle \\max |S| \\rangle$', c3)
    ]
    
    for r_idx, (r_title, key, y_lbl_ts, y_lbl_ldc, color) in enumerate(row_labels):
        ...
        ax_ts.set_title(f"Net Transformer {r_title}")
        ...
        ax_ldc.set_title(f"Net Transformer {r_title}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate thesis figures from HDF5 powerflow results.")
    parser.add_argument(
        "--fig",
        choices=["4.2a", "4.2b", "4.2b_filtered", "4.2", "4.1", "all"],
        default="all",
        help="Specify which figure to generate: '4.2a', '4.2b', '4.2b_filtered', '4.2' (both a and b), '4.1', or 'all' (default: 'all')."
    )
    parser.add_argument(
        "--out_dir",
    ...
    """scenarios = [
        {
            "id": 1,
            "label": "Pre-Urbs (EliasH)",
            "title": "Pre-Urbs (EliasH)",
            "pwrflw_dir": "/dss/dssfs05/lwp-dss-0003/pn98cu/pn98cu-dss-0001/EliasH/PostPowerflow/",
            "preurbs": True,
            "file_suffix": "_pre_EliasH",
            "color_4_2_a": "darkorange",
            "color_4_2_b": ("darkorange", "plum"),
            "color_4_1": ("darkorange", "mediumvioletred", "teal")
        },
        {
            "id": 2,
            "label": "Post-Urbs (EliasH)",
            "title": "Post-Urbs (EliasH)",
            "pwrflw_dir": "/dss/dssfs05/lwp-dss-0003/pn98cu/pn98cu-dss-0001/EliasH/PostPowerflow/",
            "preurbs": False,
            "file_suffix": "_post_EliasH",
            # Colors matching the original provided images for Post-Urbs
            "color_4_2_a": "steelblue",
            "color_4_2_b": ("steelblue", "mediumaquamarine"),
            "color_4_1": ("red", "purple", "steelblue")
        }"""
    scenarios = [
        {
            "id": 3,
            ...
            "pwrflw_dir": "/dss/dssfs05/lwp-dss-0003/pn98cu/pn98cu-dss-0001/PedroC/2nd_RUN/tests/4.PowerFlown_NegativeQ/",
            "preurbs": True,
    ...
    print("=========================================")
    print("Thesis Figure Plotting Tool - 4 Scenarios")
    print("=========================================")
    print(f"Target figures   : {args.fig}")
    ...
    for scenario in scenarios:
        ...
        plot_title = scenario.get('title', scenario['label'])
        
        if args.fig in ["4.2a", "4.2", "all"]:
            plot_figure_4_2_a(
                scenario['pwrflw_dir'], args.out_dir, scenario['preurbs'], 
                scenario['file_suffix'], scenario['color_4_2_a'], scenario['label'],
                title=plot_title
            )

        if args.fig in ["4.2b", "4.2", "all"]:
            plot_figure_4_2_b(
                scenario['pwrflw_dir'], args.out_dir, scenario['preurbs'], 
                scenario['file_suffix'], scenario['color_4_2_b'], scenario['label'],
                title=plot_title
            )
            
        if args.fig in ["4.2b_filtered", "all"]:
            plot_figure_4_2_b_filtered(
                scenario['pwrflw_dir'], args.out_dir, scenario['preurbs'], 
                scenario['file_suffix'], scenario['color_4_2_b'], scenario['label'],
                title=plot_title
            )
            
        if args.fig in ["4.1", "all"]:
            plot_figure_4_1(
                scenario['pwrflw_dir'], args.out_dir, scenario['preurbs'], 
                scenario['file_suffix'], scenario['color_4_1'], scenario['label'],
                title=plot_title
            )
